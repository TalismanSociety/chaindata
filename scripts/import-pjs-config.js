...
const main = async () => {
  ...
  const chaindataMap = Object.fromEntries(
    JSON.parse(fs.readFileSync('chaindata.json')).map((chain) => [chain.id, chain]),
  )
  ...
  // write updated files
  fs.writeFileSync('chaindata.json', await prettier.format(JSON.stringify(chaindata, null, 2), { parser: 'json' }))
  fs.writeFileSync(
