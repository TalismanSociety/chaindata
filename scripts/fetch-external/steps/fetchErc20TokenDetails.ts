...
export const getErc20ContractData = async (
  ...
  console.log(
    'Fetching token info for network:%s (%s) contractAddress:%s',
...
const updateTokenCache = async (
  ...
  if (!evmNetwork.rpcs.length) {
    console.warn('No RPCs for network', evmNetwork.name, evmNetwork.id)
    return null
  }
  const client = getEvmNetworkClient(evmNetwork)

  try {
    const { symbol, decimals, name } = (await POOL.add(() =>
      getErc20ContractData(client, contractAddress as `0x${string}`),
    )) as Awaited<ReturnType<typeof getErc20ContractData>>

    console.log('Fetched token info', symbol, decimals, name)
...
export const fetchErc20TokenDetails = async () => {
  ...
  const tokensCache = parseJsonFile<Erc20TokenCache[]>(FILE_EVM_ERC20_TOKENS_CACHE)
  ...
  await writeJsonFile(FILE_EVM_ERC20_TOKENS_CACHE, tokensCache)
}
