...
export const checkPolkadotRpcs = async () => {
  ...
  const res = await PromisePool.withConcurrency(4)
    ...
    .process(async (rpcUrl): Promise<[string, RpcHealth]> => {
      try {
  ...
  await writeJsonFile(FILE_RPC_HEALTH_POLKADOT, data, { schema: RpcHealthFileSchema })
}

const getWsRpcHealth = (wsUrl: string): Promise<RpcHealth> =>
  new Promise((resolve) => {
    let isDone = false
